...
import { detectFireflyCli } from "./firefly-cli.mjs";
  ...
  if (info.status !== 0) {
    throw new Error(
      `FireFly stack '${STACK}' is not available. Start it first:\n` +
        `  firefly init ${STACK} 1 --block-period 2 --multiparty=false -t none --sandbox-enabled=false --firefly-base-port 8000 -m scripts/firefly-manifest-v1.3.2.json\n` +
        `  firefly start ${STACK}`
    );
  }
  console.log(`FireFly stack '${STACK}' is available (using '${cli}' CLI).`);
}
...
console.log("Raters: register in the app to get a wallet and start rating");
